...
def bulkconfig_device(stc):
    ...
    dev1_handle = stc.create('emulateddevice', project, {'name': 'mydev', 'AffiliationPort-targets': port1})
    print('emulateddevice 1 handle:', dev1_handle)

    print('Configuring the emulateddevice \'mydev\'')
    ret1 = stc.bulkconfig('emulateddevice[@name="mydev"]', {'NextHopIncrement': '1.0.1.0'})
	print(ret1)
	assert str(ret1['status'])=="success"
	assert len(ret1['handles'])== 1  #['emulateddevice1']
	
    print('Configuring ethiiif and ipv4if under the emulateddevice \'mydev\'')
    ...
    print('Configuring emulateddevice \'mydev\' and its children')
